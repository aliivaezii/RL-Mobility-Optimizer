...
> React + Vite + Three.js + Leaflet/OpenStreetMap + Python RL Engine (DQN)
...
| **RL Engine** | Python 3.11 + PyTorch + NumPy | Deep Q-Network agent for route + nudge recommendation |
| **RL API** | FastAPI + Uvicorn | REST API serving RL-powered decisions to the frontend |
| **Containerisation** | Docker + docker-compose | Production-ready deployment (RL engine + frontend) |

### Architecture Decisions

- **RL backend included** — A full Python RL engine (`rl_engine/`) with FastAPI REST API can power the frontend with live route rankings, nudge selection, and behavioral profiling. The React app can run standalone with mock data or connect to the RL API for live decisions
...
```text
nexus/                             # Project root
  rl_engine/                       # Python RL Engine (DQN + FastAPI)
    __init__.py                    # Package exports
    config.py                      # User profiles, mode characteristics, RL hyperparameters
    generalized_cost.py            # Generalised Cost framework with Prospect Theory (v3 §5)
    environment.py                 # MaaS MDP environment: state, transitions, rewards (v3 §7)
    agent.py                       # Double DQN agent + Nudge Q-Network (v3 Eq. 5)
    train.py                       # Training loop, evaluation, PDF plot generation
    api.py                         # FastAPI REST API (routes, nudges, profile, carbon, trips)
    requirements.txt               # Python dependencies (torch, fastapi, numpy, matplotlib)
    README.md                      # RL engine documentation
  movewise-react/                  # React Frontend
    src/
  ...
  Dockerfile                       # Docker image for RL engine
  docker-compose.yml               # Multi-service: rl-engine + frontend + rl-train
```
...
**Train the DQN agent** (generates model weights + PDF training report):

```bash
python -m rl_engine.train
```

**Start the RL API server** (serves live route rankings, nudges, and profiles):

```bash
python -m rl_engine.api
```
...
| `/api/routes/{trip_type}` | GET | RL-ranked routes (replaces mockData.routeOptions) |
| `/api/nudge/select` | GET | Optimal nudge for current user state |
| `/api/user/profile` | GET | Full user profile with live behavioral params |
| `/api/carbon` | GET | Carbon budget with live tracking |
| `/api/user/trip` | POST | Record a trip → updates habit, phase, points |
| `/api/simulation/run` | GET | Run training simulation (demo mode) |
...
This starts:
- **rl-engine** on port 8000 (FastAPI + trained DQN)
- **frontend** on port 3000 (Vite production build served by Nginx)
- **rl-train** (one-shot training job that saves model weights)
...
## Run as Desktop App (.exe) — No Technical Knowledge Required

MoveWise can be packaged as a standalone Windows executable so anyone can launch it with a double-click — no Node.js, no terminal, no setup.

### Option A: Use the pre-built zip

1. Download **MoveWise-Windows.zip** (≈ 15 MB)
2. Extract the zip — you'll see:
   ```
   MoveWise.exe
   dist/          ← must stay next to the exe
   ```
3. **Double-click `MoveWise.exe`** — your browser opens automatically at `http://localhost:3456`
4. Press **Ctrl + C** in the console window (or close it) to stop

> **Important:** Keep the `dist/` folder in the same directory as `MoveWise.exe`. The app won't work without it.
...
This runs `vite build` → `pkg` and produces **MoveWise.exe** in the project folder. Distribute it together with the `dist/` folder.

To create a ready-to-share zip:

```powershell
Compress-Archive -Path .\MoveWise.exe, .\dist -DestinationPath .\MoveWise-Windows.zip -Force
```
